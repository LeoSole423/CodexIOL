...
def _normalize_datetime_or_date(v: Any) -> tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        # Last resort: try with date prefix.
        try:
...
def _infer_movement_kind(kind_raw: Any, description_raw: Any) -> str:
    ...
    # Keep explicit canonical values when present.
    if k in (
        "external_deposit",
        "external_withdraw",
    ...
    if any(w in s for w in ("dividendo", "renta", "cupon", "cupon", "interes")):
...
def _rows_from_json_payload(payload: Any) -> List[Dict[str, Any]]:
    ...
    if isinstance(payload, dict):
        for k in ("rows", "movements", "items", "data", "result"):
            v = payload.get(k)
...
def _movement_to_row(rec: Dict[str, Any], fmt: str, default_source: str) -> Dict[str, Any]:
    ...
    # For iol_raw we keep parser flexible; normalized path already handles canonical keys.
...
@cashflow_app.command("import-movements")
def cashflow_import_movements(
    ...
    date_from: Optional[str] = typer.Option(None, "--from", help="YYYY-MM-DD"),
    date_to: Optional[str] = typer.Option(None, "--to", help="YYYY-MM-DD"),
    replace_window: bool = typer.Option(
        False,
        "--replace-window",
        help="Delete existing movements in selected window before importing",
    ),
):
    ...
    # If replace window was requested and no explicit range was provided, infer from parsed rows.
    ...
    try:
        if replace_window and f and t:
            conn.execute(
                "DELETE FROM account_cash_movements WHERE movement_date >= ? AND movement_date <= ?",
                (f, t),
            )
            conn.commit()
