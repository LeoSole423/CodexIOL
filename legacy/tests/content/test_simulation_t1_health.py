...
def test_t1_execution_health_reports_open_fallback_and_pending_counts():
    ...
    try:
        ...
        conn.execute(
            """
            INSERT INTO simulation_pending_orders
                (run_id, signal_date, symbol, side, signal_price, status,
                 execute_date, execute_price, cancel_reason, created_at)
            VALUES (?, '2026-05-22', 'TINY', 'buy', 100, 'cancelled',
                    '2026-05-26', 100, 'unfillable_min_lot',
        ...
        assert out["cancelled_unfillable_min_lot_total"] == 1
        assert out["cancelled_unfillable_min_lot_by_family"]["daily"] == 1
        assert "pending_t1_orders" in out["flags"]
        assert "stale_pending_orders" in out["flags"]
        assert "unfillable_min_lot_cancelled" in out["flags"]
