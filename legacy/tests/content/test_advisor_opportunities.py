...
class TestAdvisorOpportunities(unittest.TestCase):
    ...
    def test_evaluate_outcomes_records_missing_symbol_forward_reason(self):
        ...
        try:
            ...
            self.assertEqual(payload["coverage"]["missing_by_reason"]["missing_symbol_forward_price"], 1)
            ...
            self.assertEqual(json.loads(row["notes_json"])["missing_reason"], "missing_symbol_forward_price")
        finally:
            conn.close()

    def test_evaluate_outcomes_uses_start_on_or_before_and_end_on_or_after_dates(self):
        from iol_advisor.continuous import evaluate_signal_outcomes
        ...
        try:
            ...
            self.assertEqual(notes["benchmark_start_date"], "2026-02-09")
            self.assertEqual(notes["benchmark_end"], 104.0)
            self.assertEqual(notes["benchmark_end_date"], "2026-02-12")
            self.assertEqual(notes["entry_price_date"], "2026-02-09")
            self.assertEqual(notes["end_price_date"], "2026-02-12")
            self.assertEqual(notes["price_policy"]["end"], "on_or_after_target")
        finally:
            conn.close()

    def test_evaluate_h5_without_forward_symbol_price_stays_missing(self):
        from iol_advisor.continuous import evaluate_signal_outcomes

        conn = self._conn()
        try:
            self._insert_minimal_candidate(conn, as_of="2026-05-22", symbol="AAPL", signal_family="new")
            conn.executemany(
                """
                INSERT INTO market_symbol_snapshots(
                  snapshot_date,symbol,market,last_price,source
                ...
                """,
                [
                    ("2026-05-22", "AAPL", "bcba", 100.0, "quote"),
                    ("2026-05-23", "AAPL", "bcba", 101.0, "quote"),
                ],
            )
            conn.commit()

            payload = evaluate_signal_outcomes(conn, as_of="2026-05-30", horizons=(5,))
            self.assertEqual(payload["coverage"]["missing_by_reason"]["missing_symbol_forward_price"], 1)

            row = conn.execute(
                """
                SELECT eval_status, forward_return_pct, excess_return_pct, hit, notes_json
                FROM advisor_signal_outcomes
                WHERE symbol='AAPL' AND horizon=5
                """
            ).fetchone()
            self.assertEqual(row["eval_status"], "missing_prices")
            self.assertIsNone(row["forward_return_pct"])
            self.assertIsNone(row["excess_return_pct"])
            self.assertIsNone(row["hit"])
            notes = json.loads(row["notes_json"])
            self.assertEqual(notes["target_date"], "2026-05-27")
            self.assertEqual(notes["missing_reason"], "missing_symbol_forward_price")
            self.assertIsNone(notes["end_price_date"])
        finally:
            conn.close()

    def test_evaluate_recalculates_legacy_zero_ok_outcome_when_forward_price_arrives(self):
        from iol_advisor.continuous import evaluate_signal_outcomes

        conn = self._conn()
        try:
            run_id, candidate_id = self._insert_minimal_candidate(
                conn, as_of="2026-05-22", symbol="AAPL", signal_family="new"
            )
            conn.executemany(
                """
                INSERT INTO market_symbol_snapshots(
                ...
                [
                    ("2026-05-22", "AAPL", "bcba", 100.0, "quote"),
                    ("2026-05-22", "BENCH", "bcba", 100.0, "quote"),
                    ("2026-05-30", "AAPL", "bcba", 120.0, "quote"),
                    ("2026-05-30", "BENCH", "bcba", 110.0, "quote"),
                ],
            )
            conn.execute(
                """
                ...
                (
                    ...
                    0.0,
                    0,
                    '{"entry_price":100.0,"end_price":100.0,"target_date":"2026-05-27"}',
                ),
            )
            conn.commit()

            payload = evaluate_signal_outcomes(conn, as_of="2026-05-30", horizons=(5,))
            self.assertEqual(payload["quality"]["ok_zero_suspect"], 1)
            self.assertEqual(payload["quality"]["ok_zero_recalculated"], 1)

            row = conn.execute(
                """
                SELECT eval_status, forward_return_pct, excess_return_pct, hit, notes_json
                FROM advisor_signal_outcomes
                WHERE candidate_id=? AND horizon=5
                """,
                (candidate_id,),
            ).fetchone()
            self.assertEqual(row["eval_status"], "ok")
            self.assertAlmostEqual(row["forward_return_pct"], 20.0, places=6)
            self.assertAlmostEqual(row["excess_return_pct"], 5.0, places=6)
            self.assertEqual(row["hit"], 1)
            notes = json.loads(row["notes_json"])
            self.assertEqual(notes["end_price_date"], "2026-05-30")
            self.assertEqual(notes["benchmark_end_date"], "2026-05-30")
        finally:
            conn.close()

    def test_scorecard_reports_raw_and_deduplicated_samples(self):
        from iol_advisor.continuous import build_variant_scorecard

        conn = self._conn()
        try:
            run_id_1, candidate_id_1 = self._insert_minimal_candidate(
                conn,
                as_of="2026-03-01",
                symbol="DUP",
                variant_id=1,
            ...
            run_id_2, candidate_id_2 = self._insert_minimal_candidate(
                conn,
                as_of="2026-03-01",
                symbol="DUP",
                variant_id=1,
            ...
            conn.executemany(
                """
                INSERT INTO advisor_signal_outcomes(
                  candidate_id, run_id, variant_id, signal_side, signal_family, symbol, as_of,
                  horizon, eval_status, forward_return_pct, excess_return_pct,
                  max_adverse_excursion_pct, max_favorable_excursion_pct, liquidity_penalty, hit, notes_json
                ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                """,
                [
                    (
            ...
            scorecard = build_variant_scorecard(conn, variant_id=1, as_of="2026-03-02", window_days=30)
            self.assertEqual(scorecard["sample_count"], 2)
            self.assertEqual(scorecard["weekly_sample_count"], 2)
            self.assertEqual(scorecard["dedup_sample_count"], 1)
            self.assertEqual(scorecard["dedup_weekly_sample_count"], 1)
            self.assertEqual(scorecard["mean_excess_return_pct"], 5.0)
            self.assertEqual(scorecard["top5_precision_buy"], 1.0)
        finally:
            conn.close()
