...
def _price_on_or_before_point(
    series: Sequence[Tuple[str, float]], target: str
) -> Optional[PricePoint]:
    latest: Optional[PricePoint] = None
    for snap, price in series:
        if snap <= target:
            latest = PricePoint(snapshot_date=str(snap), price=float(price))
        else:
            break
    return latest


def _price_on_or_after_point(
    series: Sequence[Tuple[str, float]], target: str
) -> Optional[PricePoint]:
    for snap, price in series:
        if snap >= target:
            return PricePoint(snapshot_date=str(snap), price=float(price))
    return None


def _price_on_or_before(series: Sequence[Tuple[str, float]], target: str) -> Optional[float]:
    point = _price_on_or_before_point(series, target)
    return point.price if point else None


def _benchmark_on_or_before_point(
    benchmark_by_date: Dict[str, Dict[str, float]], target: str
) -> Optional[PricePoint]:
    latest: Optional[PricePoint] = None
    for snap in sorted(benchmark_by_date):
        if snap <= target:
            price = _safe_float((benchmark_by_date.get(snap) or {}).get("mean_price"))
...
def _benchmark_on_or_after_point(
    benchmark_by_date: Dict[str, Dict[str, float]], target: str
) -> Optional[PricePoint]:
    for snap in sorted(benchmark_by_date):
        if snap >= target:
...
def _benchmark_on_or_before(
    ...
    point = _benchmark_on_or_before_point(benchmark_by_date, target)
    return point.price if point else None
...
def _is_ok_zero_suspect(row: sqlite3.Row) -> bool:
    if not row or str(row["eval_status"] or "") != "ok":
        return False
    forward = _safe_float(row["forward_return_pct"])
    excess = _safe_float(row["excess_return_pct"])
    if forward != 0.0 or excess != 0.0:
        return False
    notes = _loads_json(row["notes_json"], {})
    entry_price = _safe_float(notes.get("entry_price"))
    end_price = _safe_float(notes.get("end_price"))
    if not notes.get("end_price_date"):
        return True
    return entry_price is not None and end_price is not None and entry_price == end_price
...
def evaluate_signal_outcomes(
    ...
    *,
    ...
    skipped = 0
    ok_zero_suspect = 0
    ok_zero_recalculated = 0
    ok_zero_marked_missing = 0
    ...
    for row in candidates:
        ...
        if not run_as_of:
            ...
            continue
        start_point = _price_on_or_before_point(series, run_as_of)
        start_price = start_point.price if start_point else None
        if start_price is None or start_price <= 0:
            for horizon in horizons:
                existing = conn.execute(
                    """
                    SELECT id, eval_status, forward_return_pct, excess_return_pct, notes_json
                    FROM advisor_signal_outcomes
                ...
                suspect_existing = _is_ok_zero_suspect(existing) if existing else False
                if suspect_existing:
                    ok_zero_suspect += 1
                if existing and str(existing["eval_status"]) == "ok" and not suspect_existing:
                    continue
                ...
                notes = {
                    ...
                    "price_policy": {
                        "entry": "on_or_before",
                        "end": "on_or_after_target",
                        "benchmark_start": "on_or_before",
                        "benchmark_end": "on_or_after_target",
                    },
                }
                ...
                if suspect_existing:
                    ok_zero_marked_missing += 1
                if existing:
                    updated += 1
                else:
                    inserted += 1
            continue
        benchmark_start_point = _benchmark_on_or_before_point(benchmark_by_date, run_as_of)
        benchmark_start = benchmark_start_point.price if benchmark_start_point else None
        for horizon in horizons:
            existing = conn.execute(
                """
                SELECT id, eval_status, forward_return_pct, excess_return_pct, notes_json
            ...
            base_notes: Dict[str, Any] = {
                ...
                "entry_price_date": start_point.snapshot_date if start_point else None,
                "benchmark_start": benchmark_start,
                "benchmark_start_date": (
                    benchmark_start_point.snapshot_date if benchmark_start_point else None
                ),
                "price_policy": {
                    "entry": "on_or_before",
                    "end": "on_or_after_target",
            ...
            end_point = _price_on_or_after_point(series, target_date)
            benchmark_end_point = _benchmark_on_or_after_point(benchmark_by_date, target_date)
            end_price = end_point.price if end_point else None
            benchmark_end = benchmark_end_point.price if benchmark_end_point else None
            if end_price is None or benchmark_start is None or benchmark_end is None:
                if end_price is None:
                    reason = "missing_symbol_forward_price"
                elif benchmark_start is None:
                    reason = "missing_benchmark_start"
                else:
                    reason = "missing_benchmark_forward_price"
                base_notes["missing_reason"] = reason
                base_notes["end_price"] = end_price
                base_notes["end_price_date"] = end_point.snapshot_date if end_point else None
                base_notes["benchmark_end"] = benchmark_end
                base_notes["benchmark_end_date"] = (
                    benchmark_end_point.snapshot_date if benchmark_end_point else None
                )
                _persist_signal_outcome(
                    conn,
            ...
            path = _series_window(series, run_as_of, end_point.snapshot_date if end_point else target_date)
            ...
            notes = {
                ...
                "benchmark_start": benchmark_start,
                "benchmark_start_date": benchmark_start_point.snapshot_date if benchmark_start_point else None,
                "benchmark_end": benchmark_end,
                "benchmark_end_date": benchmark_end_point.snapshot_date if benchmark_end_point else None,
                "entry_price": start_price,
                "entry_price_date": start_point.snapshot_date if start_point else None,
                "end_price": end_price,
                "end_price_date": end_point.snapshot_date if end_point else None,
                "score_version": row["score_version"],
                "target_date": target_date,
                "price_policy": {
                    "entry": "on_or_before",
    ...
    return {
        ...
        "quality": {
            "ok_zero_suspect": ok_zero_suspect,
            "ok_zero_recalculated": ok_zero_recalculated,
            "ok_zero_marked_missing": ok_zero_marked_missing,
        },
...
def _dedup_scorecard_rows(rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    best_by_key: Dict[Tuple[Any, ...], Dict[str, Any]] = {}
    for row in rows:
        side = str(row.get("signal_side") or row.get("candidate_signal_side") or "").strip().lower()
        family = str(row.get("signal_family") or row.get("candidate_signal_family") or "").strip().lower()
        key = (
            row.get("variant_id"),
            row.get("as_of"),
        ...
        if current is None or float(row.get("score_total") or 0.0) > float(
            current.get("score_total") or 0.0
        ):
...
def build_variant_scorecard(
    ...
    *,
    ...
    dedup_rows = _dedup_scorecard_rows(rows)
    if not rows:
        return {
            ...
            "dedup_sample_count": 0,
            "dedup_daily_sample_count": 0,
            "dedup_weekly_sample_count": 0,
    ...
    scores = [_candidate_score(r) for r in dedup_rows]
    forward = [float(r.get("forward_return_pct") or 0.0) for r in dedup_rows]
    excess = [float(r.get("excess_return_pct") or 0.0) for r in dedup_rows]
    maes = [float(r.get("max_adverse_excursion_pct") or 0.0) for r in dedup_rows]
    mfes = [float(r.get("max_favorable_excursion_pct") or 0.0) for r in dedup_rows]
    daily_rows = [r for r in rows if int(r.get("horizon") or 0) == 1]
    weekly_rows = [r for r in rows if int(r.get("horizon") or 0) == 5]
    dedup_daily_rows = [r for r in dedup_rows if int(r.get("horizon") or 0) == 1]
    dedup_weekly_rows = [r for r in dedup_rows if int(r.get("horizon") or 0) == 5]
    sell_rows = [r for r in dedup_weekly_rows if str(r.get("signal_side") or "").strip().lower() == "sell"]
    false_positive = sum(1 for r in sell_rows if int(r.get("hit") or 0) == 0)
    status = "ok"
    if len(dedup_rows) < 10:
        status = "insufficient"
    ...
    return {
        ...
        "dedup_sample_count": len(dedup_rows),
        "dedup_daily_sample_count": len(dedup_daily_rows),
        "dedup_weekly_sample_count": len(dedup_weekly_rows),
        ...
        "operable_ratio": _operable_ratio(conn, variant_id=int(variant_id), as_of=as_of_v, window_days=int(window_days)),
        "top3_precision_buy": _precision_by_topn(dedup_rows, "buy", 3),
        "top5_precision_buy": _precision_by_topn(dedup_rows, "buy", 5),
        "top3_precision_sell": _precision_by_topn(dedup_rows, "sell", 3),
        "top5_precision_sell": _precision_by_topn(dedup_rows, "sell", 5),
