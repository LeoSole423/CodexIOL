...

{
  ...
  # Market universe refresh: populate market_symbol_snapshots before opportunity scoring.
  UNIVERSE_MIN=$(( (ENGINE_MIN + 5) % 60 ))
  UNIVERSE_HOUR_OFFSET=$(( (ENGINE_MIN + 5) / 60 ))
  UNIVERSE_HOUR=$(( ENGINE_HOUR + UNIVERSE_HOUR_OFFSET ))
  echo "${UNIVERSE_MIN} ${UNIVERSE_HOUR} * * 1-5 root iol advisor opportunities snapshot-universe --universe bcba_cedears >> /var/log/cron.log 2>&1"
  # Opportunities pipeline: run after universe refresh to populate advisor_opportunity_candidates.
  OPP_MIN=$(( (UNIVERSE_MIN + 10) % 60 ))
  OPP_HOUR_OFFSET=$(( (UNIVERSE_MIN + 10) / 60 ))
  OPP_HOUR=$(( UNIVERSE_HOUR + OPP_HOUR_OFFSET ))
