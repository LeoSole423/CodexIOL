...
def _cancelled_unfillable_count(conn: sqlite3.Connection, family: str, as_of: str) -> int:
    specs = {
        "daily": ("simulation_pending_orders", "simulation_runs"),
        "swing": ("swing_pending_orders", "swing_simulation_runs"),
        "event": ("event_pending_orders", "event_simulation_runs"),
    }
    pending_table, run_table = specs[family]
    return int(
        conn.execute(
            f"""
            SELECT COUNT(*)
            ...
            WHERE p.status='cancelled'
              AND p.cancel_reason='unfillable_min_lot'
              AND p.execute_date=?
...
def t1_execution_health(conn: sqlite3.Connection, as_of: str) -> Dict[str, Any]:
    ...
    for family in ("daily", "swing", "event"):
        ...
        cancelled = _cancelled_unfillable_count(conn, family, as_of)
        cancelled_unfillable_by_family[family] = cancelled
        cancelled_unfillable_total += cancelled
    ...
    if cancelled_unfillable_total:
        flags.append("unfillable_min_lot_cancelled")

    return {
        "as_of": as_of,
        "pending_by_family": pending_by_family,
        "cancelled_unfillable_min_lot_by_family": cancelled_unfillable_by_family,
        "cancelled_unfillable_min_lot_total": cancelled_unfillable_total,
...
def _daily_exit_reasons(conn: sqlite3.Connection, run_id: int) -> Dict[str, int]:
    reasons: Dict[str, int] = defaultdict(int)
    for row in conn.execute(
        """
        SELECT reason
        FROM simulation_trades
        WHERE run_id=? AND action IN ('sell', 'trim', 'exit')
        """,
        (run_id,),
    ).fetchall():
        reason = str(row["reason"] or "unknown")
        if reason.startswith("daily_exit:"):
...
def _daily_runs(conn: sqlite3.Connection, week_ago: str) -> List[Dict[str, Any]]:
    rows = conn.execute(
        ...
        FROM simulation_runs r
        JOIN simulation_bot_configs c ON c.id = r.bot_config_id
        WHERE r.mode = 'live' AND COALESCE(r.status, '') <> 'stale'
        ORDER BY r.id DESC
        """
    ).fetchall()
    ...
    for raw in rows:
        ...
        metrics_json = _loads(row.get("metrics_json"))
        item = {
            "family": "daily",
            ...
            "closed_trades": int(metrics_json.get("closed_trades") or 0),
            "daily_exit_reasons": _daily_exit_reasons(conn, row["id"]),
...
def _swing_runs(conn: sqlite3.Connection, week_ago: str) -> List[Dict[str, Any]]:
    rows = conn.execute(
        ...
        WHERE mode = 'live' AND COALESCE(status, '') <> 'stale'
        ORDER BY id DESC
        """
    ).fetchall()
    out: List[Dict[str, Any]] = []
...
def _historical_stale_summary(conn: sqlite3.Connection) -> Dict[str, Any]:
    specs = {
        "daily": (
            """
            SELECT c.name AS bot_name, r.max_drawdown_pct, r.total_trades
            FROM simulation_runs r
            JOIN simulation_bot_configs c ON c.id = r.bot_config_id
            WHERE r.mode='live' AND r.status='stale'
            """,
            "max_drawdown_pct",
            "total_trades",
        ),
        "swing": (
            """
            SELECT bot_name, max_drawdown_pct, total_trades
            FROM swing_simulation_runs
            WHERE mode='live' AND status='stale'
            """,
            "max_drawdown_pct",
            "total_trades",
        ),
    ...
    total = 0
    for family, (query, drawdown_col, trades_col) in specs.items():
        rows = conn.execute(query).fetchall()
        by_bot: Dict[str, int] = defaultdict(int)
        null_drawdown = 0
        null_trades = 0
        for row in rows:
            by_bot[str(row["bot_name"] or "unknown")] += 1
            if row[drawdown_col] is None:
                null_drawdown += 1
            if row[trades_col] is None:
                null_trades += 1
        total += len(rows)
        by_family[family] = {
            "count": len(rows),
            "by_bot": dict(sorted(by_bot.items())),
            "null_drawdown": null_drawdown,
            "null_total_trades": null_trades,
        }
...
def build_simulation_audit(
    ...
    ohlcv_dates = [
        ...
        for r in conn.execute(
            """
            SELECT DISTINCT trade_date FROM symbol_daily_ohlcv
    ...
    ohlcv_date_row = conn.execute(
        "SELECT COUNT(DISTINCT trade_date), MIN(trade_date), MAX(trade_date) FROM symbol_daily_ohlcv"
    ).fetchone()
    sym_counts = conn.execute(
        "SELECT symbol, COUNT(DISTINCT snapshot_date) AS dias FROM market_symbol_snapshots GROUP BY symbol"
    ).fetchall()
    ohlcv_sym_counts = conn.execute(
        "SELECT symbol, COUNT(DISTINCT trade_date) AS dias FROM symbol_daily_ohlcv GROUP BY symbol"
    ).fetchall()
    ...
    ohlcv_total_symbols = len(ohlcv_sym_counts)
    ohlcv_rsi_ready = sum(1 for r in ohlcv_sym_counts if r["dias"] >= 15)
    ohlcv_ma20_ready = sum(1 for r in ohlcv_sym_counts if r["dias"] >= 20)
    ohlcv_macd_ready = sum(1 for r in ohlcv_sym_counts if r["dias"] >= 35)
    ...
    for row in outcome_rows:
        if row["eval_status"] != "ok":
            continue
        if row["forward_return_pct"] != 0 or row["excess_return_pct"] != 0:
            continue
        notes = _loads(row["notes_json"])
        if not notes.get("end_price_date") or notes.get("entry_price") == notes.get("end_price"):
            ok_zero_suspect += 1
    ...
    ok_zero_suspect_total = 0
    for row in conn.execute(
        """
        SELECT forward_return_pct, excess_return_pct, notes_json
        FROM advisor_signal_outcomes
        WHERE eval_status = 'ok'
          AND COALESCE(forward_return_pct, 0) = 0
          AND COALESCE(excess_return_pct, 0) = 0
        """
    ).fetchall():
        notes = _loads(row["notes_json"])
        if not notes.get("end_price_date") or notes.get("entry_price") == notes.get("end_price"):
            ok_zero_suspect_total += 1
    ...
    h5_rows = conn.execute(
        """
        SELECT o.signal_family, o.symbol, o.as_of, o.horizon, o.signal_side,
               o.forward_return_pct, o.excess_return_pct, o.hit,
               COALESCE(c.score_total, 0) AS score_total
        FROM advisor_signal_outcomes o
        LEFT JOIN advisor_opportunity_candidates c ON c.id = o.candidate_id
        WHERE o.as_of >= ? AND o.horizon = 5 AND o.eval_status = 'ok'
        """,
        (outcomes_since,),
    ).fetchall()
    dedup_h5: Dict[tuple, sqlite3.Row] = {}
    for row in h5_rows:
        key = (
            row["signal_family"],
            row["symbol"],
        ...
        current = dedup_h5.get(key)
        if current is None or float(row["score_total"] or 0.0) > float(current["score_total"] or 0.0):
            dedup_h5[key] = row
    dedup_by_family: Dict[str, List[sqlite3.Row]] = defaultdict(list)
    for row in dedup_h5.values():
        dedup_by_family[str(row["signal_family"] or "")].append(row)
    h5_segments_dedup = [
        {
            "signal_family": family,
            "n": len(rows),
            "hits": sum(1 for r in rows if r["hit"] == 1),
            "hit_rate_pct": round(_pct(sum(1 for r in rows if r["hit"] == 1), len(rows)), 2),
            "avg_forward_return_pct": _avg([r["forward_return_pct"] for r in rows]),
            "avg_excess_return_pct": _avg([r["excess_return_pct"] for r in rows]),
        }
        for family, rows in sorted(dedup_by_family.items(), key=lambda item: (-len(item[1]), item[0]))
    ]
    ...
    historical_stale = _historical_stale_summary(conn)
    ...
    if len(ohlcv_dates) < 5:
        data_flags.append(f"ohlcv_week_lt_5:{len(ohlcv_dates)}")
    ...
    return {
        ...
        "snapshot_coverage": {
            ...
            "ohlcv_snapshot_count": len(ohlcv_dates),
        ...
        "price_warmup": {
            ...
            "ohlcv": {
                "distinct_dates": ohlcv_date_row[0],
                "first_date": ohlcv_date_row[1],
                "last_date": ohlcv_date_row[2],
                "total_symbols": ohlcv_total_symbols,
                "rsi_ready": ohlcv_rsi_ready,
                "ma20_ready": ohlcv_ma20_ready,
                "macd_ready": ohlcv_macd_ready,
                "macd_missing": ohlcv_total_symbols - ohlcv_macd_ready,
                "worst_missing_business_days_approx": max(
                    (35 - r["dias"] for r in ohlcv_sym_counts if r["dias"] < 35),
                    default=0,
                ),
            },
        },
        "signal_outcomes": {
            ...
            "ok_zero_suspect_total_historical": ok_zero_suspect_total,
        },
        "h5_segments": h5_segments,
        "h5_segments_dedup": h5_segments_dedup,
        "weekly_regime": [
            _rowdict(r)
            for r in conn.execute(
                """
