...
Los daily bots usan presets `conservative`, `balanced` y `growth`. Toman candidatos de `advisor_opportunity_candidates`, ponderan scores y restricciones del preset, y generan compras/ventas simuladas. Las entradas usan sólo candidatos `operable`; las salidas tienen una politica propia sobre holdings simulados, para no depender de sells generados contra la cartera real.

Politica de salida daily:

- `advisor_sell`: si hay sell `operable` para un holding simulado, liquida con `exit` o reduce con `trim` segun `signal_family`.
- `stop_loss`: liquida cuando el precio cae debajo del costo promedio por el porcentaje del preset.
- `take_profit`: liquida cuando el precio supera el objetivo del preset.
- `trailing_stop`: despues del hold minimo, liquida si cae desde el maximo visto desde entrada.
- `time_stop`: liquida cuando supera el maximo de dias del preset.
- `score_deterioration`: liquida si el candidato mas reciente del simbolo cae bajo el umbral de exit o queda `suppressed/rejected`.

Las salidas daily se encolan como T+1 con `engine_source=daily_exit_policy` y `reason=daily_exit:<motivo>`. Si ya hay una venta pendiente, no se duplica; si un simbolo queda marcado para salida, no se recompra en el mismo step.
...
- `cancelled_unfillable_min_lot_total`: ordenes canceladas hoy porque el monto no alcanza para comprar 1 unidad neta de costos.
...
Si aparece `unfillable_min_lot_cancelled`, el problema no es precio faltante: el bot genero una orden cuyo `amount_ars` no compra una unidad entera despues de costos/slippage. Es una cancelacion correcta de realismo; si se repite mucho, revisar sizing minimo por instrumento o filtros de candidatos.

## Auditoria Semanal

`iol simulate audit --json` es la fuente principal para la automatizacion semanal `simulation-accuracy-report`. Agrega en un solo JSON la cobertura de datos, precision H1/H5, regimen, portfolio real, health daily/swing/event, costos netos, T+1/OHLCV y flags de calidad. La cobertura separa `market_symbol_snapshots` de `symbol_daily_ohlcv` para distinguir faltantes del universo amplio de precios versus la serie OHLCV usada por simulaciones.
...
Los listados `simulate list`, `simulate swing list` y `simulate event list` excluyen por defecto runs live historicos con `status='stale'` para que la salud activa no quede contaminada por meses anteriores. Para inspeccionar historicos, usar `--include-stale`.

Los bots daily sólo operan candidatos `operable` de `advisor_opportunity_candidates`. Los candidatos `watchlist`, `manual_review`, `suppressed` y `rejected` quedan fuera de ejecución para evitar compras con evidencia insuficiente.

En daily, `win_rate_pct` mide sólo trades cerrados (`trim`/`exit`/`sell`) con PnL realizado. Si `closed_trades=0`, un `win_rate_pct=0.0` significa "sin cierres evaluables", no necesariamente que todos los buys hayan fallado.

La auditoria daily incluye `daily_exit_reasons` para explicar de donde vienen los cierres (`stop_loss`, `take_profit`, `trailing_stop`, `time_stop`, `score_deterioration` o `advisor_sell`).
...
docker exec iol-cli iol advisor opportunities snapshot-universe --universe bcba_cedears
...
11. Si hay `unfillable_min_lot_cancelled`, el sizing minimo explica la cancelacion y no queda pendiente colgada.
