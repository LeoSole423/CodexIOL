...
class BotConfig:
    ...
    max_positions: int
    # Exit policy for simulated holdings
    stop_loss_pct: float
    ...
    def from_dict(cls, d: Dict) -> "BotConfig":
        return cls(
            ...
            stop_loss_pct=float(d.get("stop_loss_pct", 0.04)),
            take_profit_pct=float(d.get("take_profit_pct", 0.08)),
            trailing_stop_pct=float(d.get("trailing_stop_pct", 0.05)),
            min_hold_days=int(d.get("min_hold_days", 1)),
            max_hold_days=int(d.get("max_hold_days", 7)),
            exit_score_threshold=float(d.get("exit_score_threshold", 30.0)),
        )
